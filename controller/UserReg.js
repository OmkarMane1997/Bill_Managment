            ...
            const encPassword = await bcrypt.hash(password,10);
               ...
               if (validator.isLength(name,{min:2 }) == false) {
                ...
                if (validator.isMobilePhone(phone)== false) {
                    return res.status(StatusCodes.BAD_REQUEST).json({ msg: " Enter Only Valid Mobile" });
                }
                ...
                return res.status(StatusCodes.BAD_REQUEST).json({ msg: ` Enter Role of User` });    
    ...
    login: async (req,res)=>{
        ...
        if (validator.isEmail(email)== false) {
            ...
            if (result.length > 0) {
                ...
                const accessToken = createAccessToken({id:result[0].id,role:userRole})
